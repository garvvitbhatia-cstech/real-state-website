@if($records->count()>0)
    ...
    <tr>
        ...
        <td>
        	@if($row->type == 'Event')
            	<a href="{{ url('/admin/add-event-images',base64_encode($row->id)) }}" class="btn btn-sm btn-success" title="Add More Images">
                	Images
            	</a>
            @endif
            <a href="{{ url('/admin/edit-blog',base64_encode($row->id)) }}" class="btn btn-sm btn-primary" title="Edit">
                <i class="bi bi-pencil"></i>
            </a>            
            <a href="javascript:void(0);" onclick="deleteData('blogs','{{ $row->id }}');" class="btn btn-sm btn-danger"  title="Delete">
